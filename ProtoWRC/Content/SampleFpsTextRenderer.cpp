    ...
    m_deviceResources(deviceResources)
{
    ...
    winrt::check_hresult(
        textFormat.as(__uuidof(m_textFormat), m_textFormat.put_void())
    );
...
void SampleFpsTextRenderer::Update(DX::StepTimer const& timer)
{
    ...
    m_text = (fps > 0) ? std::to_wstring(fps) + L" FPS(by DirectWrite)" : L" - FPS(by DirectWrite)";
    ...
    winrt::check_hresult(
        textLayout.as(__uuidof(m_textLayout), m_textLayout.put_void())
    );
