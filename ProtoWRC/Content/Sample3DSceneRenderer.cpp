...
std::future<void> Sample3DSceneRenderer::CreateDeviceDependentResources()
{
    ...
    const std::vector<byte> vertexShaderFileData = co_await DX::ReadDataAsync(L"SampleVertexShader.cso");
    winrt::check_hresult(
    ...
    const std::vector<byte>& pixelShader = co_await DX::ReadDataAsync(L"SamplePixelShader.cso");
    winrt::check_hresult(
    ...
    CD3D11_BUFFER_DESC vertexBufferDesc(sizeof(VertexPositionColor) * static_cast<UINT>(cubeVertices.size()), D3D11_BIND_VERTEX_BUFFER);
    winrt::check_hresult(
        m_deviceResources->GetD3DDevice()->CreateBuffer(
            &vertexBufferDesc,
            &vertexBufferData,
