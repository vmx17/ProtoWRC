...
// see https://learn.microsoft.com/ja-jp/windows/win32/direct3dgetstarted/complete-code-sample-for-using-a-corewindow-with-directx
...
void DX::DeviceResources::CreateDeviceResources()
{
    ...
    winrt::check_hresult(
        device.as(__uuidof(m_d3dDevice), m_d3dDevice.put_void())
    );
    //device.copy_to(reinterpret_cast<ID3D11Device**>(m_d3dDevice.put_void()));
    //device.detach();

    winrt::check_hresult(
        context.as(__uuidof(m_d3dContext), m_d3dContext.put_void())
    );
    //context.copy_to(reinterpret_cast<ID3D11DeviceContext**>(m_d3dContext.put_void()));
    //context.detach();
...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    else
    {
        ...
        winrt::check_hresult(
            m_d3dDevice.as(__uuidof(dxgiDevice), dxgiDevice.put_void())
        );
        ...
        winrt::check_hresult(
            swapChain.as(__uuidof(m_swapChain), m_swapChain.put_void())
        );
    ...
    winrt::check_hresult(
        m_swapChain.as(__uuidof(IDXGISwapChain2), spSwapChain2.put_void())
    );
...
void DX::DeviceResources::ValidateDevice()
{
    ...
    com_ptr<IDXGIDevice3> dxgiDevice;
    winrt::check_hresult(m_d3dDevice.as(__uuidof(dxgiDevice), dxgiDevice.put_void()));
